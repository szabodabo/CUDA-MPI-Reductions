...
// SDK information
static const char *shrLogFile = "MonteCarloEstimatePiInlineP.txt";
...
int main(int argc, char **argv)
{
    ...
    shrLog("Monte Carlo Estimate Pi (with inline PRNG)\n");
    shrLog("==========================================\n\n");
    ...
    try
    {
        ...
        if (shrGetCmdLineArgumentstr(argc, (const char **)argv, "precision", &value))
        {
            ...
            {
                shrLogEx(LOGBOTH | ERRORMSG, 0, "specified precision (%s) is invalid, must be \"single\" or \"double\".\n", value);
...
void runTest(int argc, const char **argv)
{
    ...
    try
    {
        ...
        {
            shrLogEx(LOGBOTH | ERRORMSG, 0, "could not get device count (%s).\n", cudaGetErrorString(cudaResult));
...

        {
            ...
            if (shrGetCmdLineArgumentstr(argc, argv, "block-size", &value))
            {
                ...
                {
                    shrLogEx(LOGBOTH | ERRORMSG, 0, "specified block size (%d) is invalid, must be a power of two (see reduction function).\n", test.threadBlockSize);
                    throw invalid_argument("block-size");
                }
            }
            else
            ...
            if (shrGetCmdLineArgumentstr(argc, argv, "seed", &value))
            {
                // Check requested seed is valid
                test.seed = (unsigned int)atoi(value);
                if (test.seed == 0)
                {
                    shrLogEx(LOGBOTH | ERRORMSG, 0, "specified seed (%d) is invalid, must be non-zero.\n", test.seed);
