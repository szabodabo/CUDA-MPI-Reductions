...
int main(int argc, char** argv) {
	...
	if (sFilename.length() == 0 || g_bQATest) {
		char *filePath = findFilePath(default_image, argv[0]);
		if (filePath) {
			sFilename = filePath;
		} else {
	...
	if( g_bDisplay ) initGL(&argc, argv, width, height);
	...
	if( g_bDisplay ) glutMainLoop();

	int qaStatus = QA_PASSED;

	if( g_bQATest ) {
		qaStatus = verifyResult(findFilePath(gold_image, argv[0])) ? QA_PASSED : QA_FAILED;
	}
	...
	shrQAFinishExit(argc, (const char **)argv, qaStatus);
}
...
void display() 
{
	...
	glTexCoord2f(1.0, 0.0); glVertex3f((GLfloat)width, 0.0, 0.5);
	glTexCoord2f(1.0, 1.0); glVertex3f((GLfloat)width, (GLfloat)height, 0.5);
	glTexCoord2f(0.0, 1.0); glVertex3f(0, (GLfloat)height, 0.5);
	glEnd();	

	glBindTexture(GL_TEXTURE_2D, 0);

	// Plot User Rectangle
	if( display_mode == 1) plotUserRect();
